...
- Add `LogContextKey` enum for known `LogContext` property keys (TBD)
- Add `PRIORITY_CONTEXT_KEYS`, `DURATION_UNIT_SUFFIX`, and `SUCCESS_INDICATOR` named constants to logger (TBD)
...
- Replace magic strings in `logger.ts` with `LogContextKey` enum and named constants (TBD)
