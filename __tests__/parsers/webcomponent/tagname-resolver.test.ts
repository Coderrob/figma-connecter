...
describe('resolveTagName', () => {
  ...
  it('should resolve tag name via register() with re-exported constants', () => {
    ...
    const widgetConstantsPath = path.join(componentDir, 'widget.constants.ts');
    const namespaceConstantsPath = path.resolve(componentDir, '../../utils/tag-name/constants.ts');
    ...
    jest
      .spyOn(fs, 'existsSync')
      .mockImplementation((p: fs.PathLike) => fileMap.has(path.resolve(p.toString())));
  ...
  it('should resolve tag name from constants fallback when ./constants.ts is missing', () => {
    ...
    const indexPath = path.join(componentDir, 'index.ts');
    const fallbackConstantsPath = path.join(componentDir, 'fallback.constants.ts');
    const namespaceConstantsPath = path.resolve(componentDir, '../../utils/tag-name/constants.ts');
  ...
  it('should warn when tag name identifier cannot be resolved', () => {
    ...
    expect(result.tagName).toBe('component');
    expect(
      result.warnings.some((warning) => warning.includes('Unable to resolve tag name identifier')),
    ).toBe(true);
  ...
  it('should resolve tag names through export star re-exports', () => {
    ...
    fs.writeFileSync(path.join(componentDir, 'constants.ts'), "export * from './values';", 'utf8');
  ...
  it('should warn when register receives unsupported expressions', () => {
    ...
    expect(result.tagName).toBe('component');
    expect(result.warnings.some((warning) => warning.includes('Unsupported register()'))).toBe(
      true,
    );
  ...
  it('should fall back when no register call exists', () => {
    ...
    fs.writeFileSync(path.join(componentDir, 'index.ts'), 'export const value = 1;', 'utf8');
  ...
  it('should warn when register is missing a tag name argument', () => {
    ...
    expect(result.tagName).toBe('component');
    expect(
      result.warnings.some((warning) => warning.includes('did not include a tag name argument')),
    ).toBe(true);
  ...
  it('should handle export targets that cannot be read', () => {
    ...
    fs.writeFileSync(constantsPath, 'export { TAG_NAME } from "./values";', 'utf8');
    fs.writeFileSync(valuesPath, 'export const TAG_NAME = "my-unreadable";', 'utf8');
  ...
  it('should ignore local exports that cannot be resolved', () => {
    ...
    fs.writeFileSync(path.join(componentDir, 'constants.ts'), 'export { TAG_NAME };', 'utf8');
